 ...
 *
 */
public class JmsStoutFeedbackListener extends BaseTimer {
	...
	protected String getLogPrefix() {
		return "JmsStoutFeedbackListener:出库业务反馈处理：";
	}

	@Override
	protected String getRequestName() {
		return JmsConstant.STOUTFEEDBACK;
	}

	@Override
	protected String getQuerySql() {
		return  "select * from inf_erp_imp_ck_bill where yew_type = '销售出库' and (nvl(zt,'N') = 'N' or (zt = 'Y' and process = '0'))";
	}
	...
	protected void updateData(Connection con, ResultSet resultSet) throws Exception {
		...
		while (resultSet.next()) {
			...
			Integer dj_sort = resultSet.getInt("Dj_sort");// 行号
			String spid = resultSet.getString("spid");// 分公司商品内码
			log.info("JmsStoutFeedbackListener.updateData,数据写入jms，更新中间表状态");
			String sql = "update inf_erp_imp_ck_bill set zt = 'Y',process = '0' where yew_type = '销售出库' and djbh = '"+djbh+ "' and Dj_sort = " +dj_sort+" and spid = '"+spid+"' and dwbh = '"+dwbh+"' and nvl(process, '-1') != '1'";
			preparedStatement = con.prepareStatement(sql);
			log.info("JmsStoutFeedbackListener.updateData,数据写入jms，更新中间表状态,执行sql："+sql);
			preparedStatement.executeUpdate();
			log.info("JmsStoutFeedbackListener.updateData,sql="+sql+",执行完成 =====================");
